...
def _find_simple_name(
    ...
    # Step 7c: If not found and we stopped before root (see Step 7b above)
    ...
    if not found and current_scope.parent:
        found = _find_local(name, scope.root)
        if found and (
            not isinstance(found.leaf, ast.Class)
            or found.leaf.type
            not in (
                "type",
                "function",
...
def _find_inherited(
    ...
    for extends in scope.extends:
        ...
        if extends_scope:
            ...
            if found:
                if found.leaf.name in ast.Tree.BUILTIN_TYPES:
                    return found
...
def instantiate(class_name: str, class_tree: ast.Tree) -> ast.InstanceClass:
    ...
    """
    class_path = find_name_path(class_name, class_tree)
    if not class_path:
    ...
    # Instantiate the original path and set parent so instance.full_reference() is correct
    path = _instantiate_path_partially(class_path.path, ast.ClassModification(), instance_tree)
    instance.parent = path.leaf
    return instance
...
def _instantiate_extends_single(
    ...
    extend_mod = _append_modifications(extends.class_modification, modification_environment)
    extends_instance_path = _instantiate_path_partially(extends_class_path, extend_mod, parent.root)
    ...
    extends_instance = _instantiate_class(extends_instance, extend_mod, extends_instance.parent)
...
def _instantiate_symbol(
    ...
    if not isinstance(symbol.type, ast.InstanceClass):
        ...
        if symbol is not symbol_before_redeclare:
            ...
            type_parent = parent
        symbol_type_path = find_name_path(symbol.type, type_parent)
        ...
        if isinstance(symbol_type, ast.InstanceElement):
            ...
            instance_path = _instantiate_path_partially(
                symbol_type_path.path, ast.ClassModification(), parent.root
            )
    ...
    for arg in instance.modification_environment.arguments:
        if isinstance(arg.value, ast.ElementModification):
            ...
            if arg.value.component.child:
                ...
                if instance.name in InstanceTree.BUILTIN_TYPES:
                    mod.arguments.append(arg)
                else:
...
def _instantiate_path_partially(
    ...
    different modifications) which is used as the dict key.
    ...
    parent = root
    child_mod = _append_modifications(modification_environment)
    for element in path:
        if isinstance(element, ast.Tree):
            continue
        if isinstance(element, ast.ExtendsClause):
            instance = _instantiate_extends_single(element, child_mod, parent)
        ...
        parent = instance
        child_mod = _append_modifications(parent.modification_environment, child_mod)
...
def _apply_redeclares(
    ...
    apply_args = []
    if isinstance(redeclare, ast.ShortClassDefinition):
        if redeclare.class_modification is not None:
