...
Usage examples::
    ...
    doc/doc/doc/pymoca_011_vs_new_run_all.py

    # Force a fresh clone and venvs (e.g. after a large rebase).
    doc/doc/doc/pymoca_011_vs_new_run_all.py --fresh

    # Use a different clone directory.
    doc/doc/doc/pymoca_011_vs_new_run_all.py --clone-dir ~/tmp/pymoca-nb

    # Set up the environments but skip notebook execution.
    doc/doc/doc/pymoca_011_vs_new_run_all.py --no-execute
...
def ensure_clone(clone_dir: Path, source: Path, branch: str, fresh: bool) -> None:
    ...
    if not clone_dir.exists():
        ...
        _run(["git", "-C", str(clone_dir), "checkout", branch])
    else:
        # Sync to the exact commit the worktree is on.
        print(f"[clone] Fetching updates from {source}")
        _run(["git", "-C", str(clone_dir), "fetch", str(source), branch])
        _run(["git", "-C", str(clone_dir), "checkout", branch])
        _run(["git", "-C", str(clone_dir), "reset", "--hard", head_sha])
...
def main() -> None:
    ...
    ensure_clone(args.clone_dir, source, branch, args.fresh)
    print()
